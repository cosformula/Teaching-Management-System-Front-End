<template>
  <el-table :data="courseWaited" highlight-current-row height="500" border>
    <el-table-column label="操作" width="150">
      <template scope="scope">
        <el-tooltip effect="dark" content="添加到课表" placement="left">
          <el-button size="small" v-if="scope.row.status=='待加入'" @click="addSchedule(scope.row)" icon="el-icon-circle-plus"></el-button>
          <!-- <el-button size="small" v-if="scope.row.status=='已加入'" @click="selectCourse(scope.row)">选课</el-button> -->
        </el-tooltip>
        <el-tooltip effect="dark" :content="scope.row.status=='待加入'?'删除':'从课表删除'" placement="right">
          <el-button size="small" type="danger" icon="el-icon-remove-outline" @click="delCourse(scope.row)"></el-button>
          <!-- <el-button size="small" v-if="scope.row.status=='已选入'" type="danger" @click="quitCourse(scope.row)">退课</el-button> -->
        </el-tooltip>
      </template>
    </el-table-column>
    ...
    <el-table-column label="教师" width="80">
      <template slot-scope="scope">
        <span style="white-space:nowrap;"> {{ scope.row.teacher.name }}</span>
      </template>
    </el-table-column>
...
export default {
  ...
  methods: {
    ...
    quitCourse: function(row) {
      // console.log(row)
      this.$emit('quitCourse', row)
    }
  }
}
</script>
